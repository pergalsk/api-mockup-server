 ...
 * @param {Object} watchPaths Object containig routes and/or database paths to watch for changes.
 * @param {string} watchPaths.routes String containing path to file with routes definitions.
 * @param {string} watchPaths.database String containig path to directory with data files.
 * @example
 *  const watcher = require('./lib/watch');
 *  watcher.watch({
 *   database: './db', // database should be a folder
 *   routes: './routes.js' // routes shuold be a file
 * });
 */
function watch(watchPaths) {
  ...
  if (utils.isString(routes)) {
    ...
    routesWatcher
      .on('change', (path) => log.warn(_t('FILE_OR_DIR_CHANGED_ROUTES', { path })))
      .on('add', (path) => log.warn(_t('FILE_OR_DIR_ADDED_ROUTES', { path })));
  }
