...
function coloredLog(chalkColor) {
  return (messages) => {
    if (!Array.isArray(messages)) {
      console.log(chalk[chalkColor](reduceNewLines(messages)));
      return;
    }

    messages.forEach((message) => {
      console.log(chalk[chalkColor](reduceNewLines(message)));
    });
  };
}

let lastMessageEndedWithNewLine = false;
function reduceNewLines(message) {
  let result = message;
  // reduce to one new line
  result = result.replace(/\n+/g, NEW_LINE);
  // if last time there was a new line at the end,
  // remove first new line character from current message
  // to avoid double new lines
  if (lastMessageEndedWithNewLine && message.startsWith(NEW_LINE)) {
    result = result.slice(NEW_LINE.length); // remove first new line character
  }
  // remember if message ends with new line character
  lastMessageEndedWithNewLine = message.endsWith(NEW_LINE);
  return result;
}
...
function serverConfig(prefix, target, port) {
  ...
  prefix && notice(LEFT_PAD + chalk.white(_t('GLOBAL_PREFIX')) + ' ' + prefix);
  notice(LEFT_PAD + chalk.white(_t('PROXY_TARGET')) + '  ' + target);
  notice(LEFT_PAD + chalk.white(_t('SERVER_PORT')) + '   ' + port + NEW_LINE);
}
 ...
 * @returns {void}
 */
function routesList(routesDisplayList, inactive = false) {
  routesDisplayList.forEach((route, index) => {
    const { applyIf, method, status, key, path, interceptor } = route;

    const c = {
      method: chalk[inactive ? 'gray' : 'greenBright'](method.toUpperCase()),
      key: chalk[inactive ? 'gray' : 'yellowBright'](key || '-'),
      path: chalk[inactive ? 'gray' : 'whiteBright'](path),
      status:
        status < 400
          ? chalk[inactive ? 'gray' : 'whiteBright'](status)
          : chalk[inactive ? 'gray' : 'redBright'](status),
    };

    const hasApplyIf = typeof applyIf === 'function';
    const badges = `${interceptor ? '&' : ' '}${hasApplyIf ? '*' : ' '}`;
    const message = `${badges} ${c.method} ${c.status} ${c.key} ${c.path}`;
  ...
  const displayLegend = routesDisplayList.some(
    ({ applyIf, interceptor }) => interceptor || typeof applyIf === 'function',
  );

  if (displayLegend) {
    debug(LEFT_PAD + _t('BADGES_LEGEND'));
  }
}
...
function routerConfig(
  ...
  if (suspended != null && !isFileChange) {
    return;
  }
  // do some extended console output to inform user
  /* if (suspended === 'TURNING_ON') {
    emphasize(_t('ROUTES_LIST'));
    warn(LEFT_PAD + _t('SUSPENDED_LIST'));
    return;
  } */

  emphasize(NEW_LINE + _t('ROUTES_LIST'));

  if (activeRoutes.length === 0) {
    const textCode =
      proxy && proxy.server && target
  ...
  routesList(routesDisplayList, false);
}

function suspendedMessage(suspended, isFileChange) {
  if (suspended == null || isFileChange) {
    return;
  }

  if (suspended === 'TURNING_ON') {
    warn(NEW_LINE + _t('MOCKING_SUSPENDED') + NEW_LINE);
  }
  if (suspended === 'TURNING_OFF') {
    notice(NEW_LINE + _t('MOCKING_RESUMED') + NEW_LINE);
  }
}
