...
function serverStart() {
  hrSeparator();
  serverMsg(chalk.whiteBright(_t('STARTING_SM_SERVER')));
}
...
function serverListen(serverOptions, routesList) {
  ...
  info(
    prefix() + _t('SERVER_LISTENING_AT') + chalk.greenBright(`http://localhost:${port}`) // TODO: real server
  );

  proxy &&
    proxy.server &&
    info(prefix() + _t('PROXY_SERVER_AT') + chalk.yellowBright(proxy.server));
