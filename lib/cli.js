...
function portAction({ serverOptions }) {
  log.emphasize(NEW_LINE + _t('SERVER_PORT'));
  log.notice(LEFT_PAD + serverOptions.port);
}

function prefixAction({ serverOptions }) {
  log.emphasize(NEW_LINE + _t('GLOBAL_PREFIX'));
  log.notice(LEFT_PAD + serverOptions.prefix);
}
...
function targetAction({ serverOptions }) {
  log.emphasize(NEW_LINE + _t('PROXY_TARGET'));
  if (getState().target) {
    log.notice(LEFT_PAD + getState().target);
    return;
  }
  ...
  if (
    ...
    log.notice(LEFT_PAD + _t('WITHOUT_PROXY'));
  } else {
    log.notice(LEFT_PAD + _t('PROXY_NOT_CONFIGURED'));
  }
}
