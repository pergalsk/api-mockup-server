 ...
 * @param {boolean} suspended Indicates if the routes mocking is suspended
 * @returns {Promise<Function>} ExpressJS HTTP Proxy Middleware
 */
function getProxy(serverOptions, target, suspended) {
  ...
  suspended == null && log.serverMsg(_t('PROXY_CONFIGURED'));
...
function shouldApplyCheck(req, context, pathMatch, skipInterceptors) {
  ...
  if (!isFn(applyIf)) {
    return result; // route definition should be applied
  }
