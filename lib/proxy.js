...
async function getProxy(serverOptions) {
  ...
  let proxyOpt = {
    target,
    onProxyRes: onProxyRes(encoding, contexts),
  };
  ...
  return createProxyMiddleware(getFilterFn(contexts), { ...defaultOptions.proxy, ...proxyOpt });
}

function onProxyRes(encoding, contexts) {
  return responseInterceptor(async (responseBuffer, proxyRes, req, res) => {
    ...
    try {
      ...
      data = context.callback(req, proxyRes, data);
