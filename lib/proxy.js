...
/**
 * Determines the human-readable (localized) text to display for the current proxy target.
 * @param {Object} serverOptions - Server configuration options (required).
 * @returns {string} Localized text describing the effective proxy target.
 */
function getProxyTargetText(serverOptions) {
  let targetText = _t('PROXY_NOT_CONFIGURED');

  if (getState().target) {
    targetText = getState().target;
  } else {
