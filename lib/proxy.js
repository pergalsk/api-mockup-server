...
function getOnProxyResFn(contexts, serverOptions) {
  return responseInterceptor(async (responseBuffer, proxyRes, req, res) => {
    ...
    const context = contexts.find(getMatchRouteFn(req, req.originalUrl, false));
    ...
    try {
      ...
      // TODO: investigate callback params
      // const { params, query, body } = req;
      // const callbackProps = { params, query, body, data: parsedData, req };

      const modifiedData = context.callback(req, proxyRes, parsedData /* callbackProps */);
...
function getMatchRouteFn(req, pathname, negateApplyIf) {
  return (context) => {
    ...
    const applyCheck = applyIfMatch(req, context, pathMatch, negateApplyIf);
    if (applyCheck) {
      return false;
    }
 ...
 * @param {boolean} negateApplyIf - If true, the result of the `applyIf` check is negated.
 * @returns {boolean} - Returns true if the condition is met (or not met, if negated); otherwise, false.
 */
function applyIfMatch(req, context, pathMatch, negateApplyIf) {
  const { query, body } = req;
  const { params } = pathMatch;
  const applyIfProps = { params, query, body, req };

  const applyIf = isFn(context.applyIf) ? context.applyIf(applyIfProps) : true;
  return negateApplyIf ? !applyIf : applyIf;
}
