...
function routeFn(data, routeOptions, serverOptions) {
  return async (req, res) => {
    ...
    const { path, status, method, key, bypass, delay, callback } = routeOptions;
    ...
    if (bypass === true && bypassOpt.server) {
      const externalRoute = bypassOpt.server + (bypassOpt.prefix || '') + path;
      log.info('External call -> ' + externalRoute);

      extData = await fetch(externalRoute)
        .then((res) => res.json())
        .then((json) => json);
    }
    ...
    if (callback) {
      data = await callback(req, res, data, extData);
    }
