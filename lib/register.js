...
function routeFn(routeOptions, serverOptions) {
  return async (req, res) => {
    ...
    setTimeout(() => {
      ...
      isFunction(callback) && log.callbackRoute(req.method, res.statusCode, key, req.originalUrl);
    }, interval);
  };
}
 ...
 * @param {string} path - The path to the module to clear from the cache.
 * @returns {void}
 */
function clearRequireCache(path) {
  ...
  try {
    ...
    log.error(_t('ERROR_WHILE_CLEARING_REQUIRE_CACHE', { path }));
  }
}
