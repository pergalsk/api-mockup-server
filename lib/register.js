...
const { MOCK_HEADER_NAME, NEW_LINE, LEFT_PAD } = require('./constants');
...
function getRouter(serverOptions, isFileChange) {
  const router = express.Router();

  const { routes, prefix, proxy } = serverOptions;
  const { suspended, target } = getState();

  let acceptedRoutesNum = 0;

  // all routes file entries
  let allRoutes = suspended == null || suspended === 'TURNING_OFF' ? getAllRoutes(routes) : [];
  ...
  activeRoutes.forEach((route) => {
    ...
    const interceptor = isProxyInterceptor(routeOptions, target);

    acceptedRoutesNum++;
    routesDisplayList.push({ method, status, key, path: fullPath, applyIf, interceptor });
  ...
  if (suspended == null || isFileChange) {
    ...
    if (suspended === 'TURNING_ON') {
      ...
      log.routesList(routesDisplayList, target);
      log.debug(
        LEFT_PAD +
    ...
    if (suspended === 'TURNING_ON') {
      log.warn(NEW_LINE + _t('MOCKING_SUSPENDED'));
    }
    if (suspended === 'TURNING_OFF') {
      log.notice(NEW_LINE + _t('MOCKING_RESUMED'));
    }
  }

  return { routerMiddleware: router, routesDisplayList };
}
...
function routeFn(routeOptions, serverOptions) {
  return async (req, res) => {
    ...
    if (isFn(callback)) {
      ...
      try {
        resultData = await callback(callbackProps);
