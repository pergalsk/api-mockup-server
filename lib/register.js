...
function getRouter(serverOptions) {
  ...
  routesList.forEach((route) => {
    ...
    if (!method) {
      log.warn(_t('WARNING_NO_VALID_METHOD'));
  ...
  if (routesList.length === 0) {
    log.warn(_t('WARNING_MISSING_ROUTE_DEFINITIONS'));
  }
  ...
  routesDisplayList.forEach((route) => {
    ...
    log.warn(
      (applyIf ? ' * ' : '   ') +
        `${route.method} ${route.status} ${route.key || '-'} ${route.path}`
    );
  });

  asterisk && log.debug('   ' + _t('ROUTES_MARKED_WITH_ASTERISK'));
