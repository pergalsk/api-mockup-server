...
const { getProxyTargetText } = require('./proxy');
 ...
 * @param {Object} serverOptions - Server options for logging purposes.
 * @param {Array<Object>} routesDisplayList - List of routes for logging purposes.
 ...
 * }}
 */
function ServerWrapper(server, serverOptions, routesDisplayList, stats, options = {}) {
  ...
  function configureServer(srv) {
    ...
    srv.on('listening', () => {
      const { target, suspended, isRestart, isFileChange } = getState();

      if (isRestart) {
        return;
      }

      const { activeRoutes, acceptedRoutesNum, allRoutes } = stats;
      log.serverConfig(prefix, getProxyTargetText(serverOptions), port);
      log.routerConfig(
        serverOptions,
        routesDisplayList,
        activeRoutes,
    ...
    srv.on('error', (err) => {
      if (err.code === 'EADDRINUSE') {
        log.error(NEW_LINE + _t('ERROR_PORT_IN_USE', { port }));
